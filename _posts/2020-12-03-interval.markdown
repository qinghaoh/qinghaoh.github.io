...
public int videoStitching(int[][] clips, int time) {
    ...
    // prev: [0, prev] are covered by previous levels
    // curr: reach by intervals in this level
    int index = 0, prev = 0, curr = 0, res = 0;
    while (prev < time) {
        while (index < clips.length && clips[index][0] <= prev) {
            curr = Math.max(curr, clips[index++][1]);
        }
...
We can also bucket sort the input array to improve time complexity:
...
public int minGroups(int[][] intervals) {
    ...
    // stores the end of each group
    Queue<Integer> pq = new PriorityQueue<>();
    for (int[] i : intervals) {
        if (!pq.isEmpty() && pq.peek() < i[0]) {
            // the current interval doesn't overlap the top group
            // updates the end of the group
            pq.poll();
        }

	// else, adds the current interval to represent a new group
        pq.offer(i[1]);
    }
    return pq.size();
}
...
public int jobScheduling(int[] startTime, int[] endTime, int[] profit) {
    ...
    // sorts indices by end time
    Arrays.sort(indices, Comparator.comparingInt(i -> endTime[i]));

    // end time : max profit by the end time
    ...
    for (int i : indices) {
        ...
        if (curr > dp.lastEntry().getValue()) {
            ...
            // it guarantees the map value increases as endTime (map key) increases
