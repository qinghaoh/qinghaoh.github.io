...
public long minimumTotalCost(int[] nums1, int[] nums2) {
    ...
    // e.g. [3, *, *, 1, *, 2]
    //  swap #1: [1, *, *, 3, *, 2], cost += 0 + 2 = 2
    //  swap #2: [2, *, *, 1, *, 3], cost += 0 + 5 = 5
    //  notices 2 and 3 are not mutually swapped.
    //  instead, neither of them are in its original position, and that's all we need
    for (int i = 0; i < n; i++) {
    ...
    // if majority element exists, we can't move all to-displace elements to a position with different value
