...
Partition a dataset into groups. Each DFS level iterates the mask bits and tentatively adds each unused bit.
...
private boolean dfs(int[] nums, int mask, int subsetSum, int target) {
    ...
    for (int i = 0; i < n; i++) {
        if ((mask & (1 << i)) == 0) {
            // assigns i-th bit
            int next = mask | (1 << i);
            if (nums[i] + subsetSum <= target) {
                if (dfs(nums, next, (subsetSum + nums[i]) % target, target)) {
                    return true;
                }
            }
        }
