...
_G. R. Franssens, On a number pyramid related to the binomial, Deleham, Eulerian, MacMahon and Stirling number triangles, Journal of Integer Sequences, Vol. 9 (2006), Article 06.4.1._

若以\\(\cosh^m(x)\\)为指数母函数（[Generating function](https://en.wikipedia.org/wiki/Generating_function)）进行麦克劳林（[Maclaurin](https://en.wikipedia.org/wiki/Taylor_series)）展开，可得**Even multinomial parity numbers**：
...
$$p_{X}(k = \frac{N}{2}-2) = p_{X}(k = \frac{(N-1)-3}{2}) = \frac{1}{4}-\frac{e_{N}^{4}}{4^N}$$
